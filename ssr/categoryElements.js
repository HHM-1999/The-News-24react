...
const ldJsonElements = require("./categoryComponents/ldJsonElements");
...
const moreNewsSectionElements = require("./categoryComponents/moreNewsElements");

async function categoryElement(catID, catSlug, catTitle) {
    let ldJsonComponent;
    try { ldJsonComponent = await ldJsonElements.ldJsonElement(catSlug, catTitle); }
    catch (err) { console.log('ldJsonComponent error'); return ''; }

    let leadComponent;
    let leadContentIDs = [];
    try { const leadNewsRes = await bnConfig.query( 'SELECT bn_contents.ContentID, bn_bas_categories.CategoryID, bn_bas_categories.CategoryName, bn_bas_categories.Slug, bn_contents.ContentHeading, bn_contents.ContentBrief, bn_contents.ImageThumbPath, bn_contents.ImageSmPath, bn_contents.ImageBgPath, bn_contents.URLAlies, bn_contents.VideoID, bn_contents.ShowVideo, bn_contents.VideoPath, bn_contents.VideoType, bn_contents.VideoSource, bn_contents.created_at FROM `bn_contents` JOIN bn_positions ON bn_contents.ContentID=bn_positions.ContentID JOIN bn_bas_categories ON bn_positions.ForeignID=bn_bas_categories.CategoryID WHERE bn_positions.HomePosition=2 AND bn_positions.PositionType=1 AND bn_positions.ForeignID=? AND bn_positions.Position>1 AND bn_contents.Deletable=1 AND bn_contents.ShowContent=1 ORDER BY bn_positions.Position ASC LIMIT 5', [catID] );
        ...
        for (let i = 0; i < leadNewsRes.length; i++) {
            if(i==0){
                cathtml1 += `<div class="DCatLeadTop">
                    <a href="${'/' + leadNewsRes[i].Slug + '/news/' + leadNewsRes[i].ContentID}">
                        <div class="row">
    ...
    try { moreNewsComponent = await moreNewsSectionElements.moreNewsSectionElement(catID, catSlug, leadContentIDs, bnConfig); }
    catch (err) { console.log('moreNewsComponent error'); return ''; }
    
    let categoryLatestComponent;
    try { categoryLatestComponent = await latestPopularSectionElements.categoryLatestSectionElement(catID, catSlug); }
    ...
    let elements = `${ldJsonComponent}<main>
        <div class="container">
            <h2 class="DTitle">
                <a href="/${catSlug}">
                    <span class="DTitleInner"><span class="DTitleInnerBar"><span>${catTitle}</span></span></span>
                </a>
            </h2>
            <section>
                <div class="row">
                    <div class="col-lg-9 col-sm-12">
                        ${catSlug === 'countries' ? staticElements.districtTab() : ''}
                        ${leadComponent}
                    </div>
            ...
            <section>
                <div class="row">
                    <div class="col-lg-9 col-sm-12">
                        <h2 class="LatestNewsH mt-4">${catTitle} বিভাগের সব খবর</h2>
