...
async function popularElement() {
    try { const popularRes = await mediaConfig.query( 'SELECT * FROM tv_webtvs JOIN video_totalhits ON video_totalhits.VideoID=tv_webtvs.WebTVID WHERE tv_webtvs.Deletable=1 AND created_at >= (CURDATE() - INTERVAL 1 MONTH ) ORDER BY video_totalhits.TotalHit DESC LIMIT 10' );
