...
async function moreNewsSectionElement(catID, catSlug, contentIDs, bnConfig) {
    try {
        const moreNewsRes = await bnConfig.query( 'SELECT bn_contents.ContentID, bn_contents.ContentHeading, bn_contents.ContentBrief, bn_contents.ImageThumbPath, bn_contents.ImageSmPath, bn_contents.ImageBgPath, bn_contents.URLAlies, bn_contents.VideoID, bn_contents.ShowVideo, bn_contents.VideoPath, bn_contents.VideoType, bn_contents.VideoSource, DATE_FORMAT(bn_contents.created_at, "%W, %e %M %Y, %H:%i") as create_date, DATE_FORMAT(bn_contents.updated_at, "%W, %e %M %Y, %H:%i") as updated_date FROM bn_contents WHERE FIND_IN_SET(?, bn_contents.CategoryIDs) > 0 AND bn_contents.Deletable=1 AND bn_contents.ShowContent=1 AND ContentID NOT IN (?) ORDER BY bn_contents.ContentID DESC LIMIT 8 OFFSET 0', [catID, contentIDs] );
